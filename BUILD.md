...
This document provides instructions for building the Blitz3D WASM compiler on
...
If you encounter errors like `missing SDK` or `target not found` when using nix,
ensure your `SDKROOT` is set correctly:
...
- [Visual Studio 2022](https://visualstudio.microsoft.com/) with "Desktop
...
The `web/` frontend can be built and run via Deno tasks:
