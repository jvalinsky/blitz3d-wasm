...
AAText.bb, DrawPortals.bb, NPCs.bb, Achievements.bb
...
**Root cause:** Statement branches leave values on the WASM stack that aren't
consumed. Common scenarios:
...
**Root cause:** Some functions expect arguments in a specific order but the
compiler generates them in wrong order. This happens when:
...
**Current status:** These call the base function (e.g., `LoadImage`) but may
have different signatures.

**Location:** `Sources/Compiler/CodeGen/CodeGenerator.swift` (builtin function
registration)
...
**Status:** Need to add stubs or find actual include files for the Map Creator
module.
...
WASM requires that all branches of an if/loop have the same stack effect.
Currently:
...
When generating function call arguments, type promotion should happen BEFORE
pushing values:
...
- `Sources/Compiler/CodeGen/ExpressionGeneration.swift` - Fixed function call
  returns
- `Sources/Compiler/CodeGen/StatementGeneration.swift` - Fixed elseIf handling,
  For loop step
