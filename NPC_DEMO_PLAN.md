...
Detail textures 2-3 and cube textures require custom shaders (not loaded by
...
1. **Bind pose not applied**: Three.js requires skeleton to be in bind pose when
   `mesh.bind()` is called
2. **Inverse bind matrices**: May be computed incorrectly or not applied
   properly
3. **Bone hierarchy**: Bones need proper parent-child relationships established
   before binding
...
**Key insight**: Blitz3D animations are frame-based, not time-based. The runtime
needs to:
