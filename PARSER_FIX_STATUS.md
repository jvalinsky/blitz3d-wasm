...
Parser incorrectly handles parenthesized expressions in function arguments at statement level:
- `Text (x+width/2),(y+height/2-1), txt, True, True` - should parse as 5 arguments
- Currently parsed as `Text(x+width/2)` function call, leaving `, (y+height/2-1), txt, True, True` unparsed

## Root Cause

`parsePostfixExpression()` treats `identifier(` as function call syntax, consuming the parentheses.
In statement context, parentheses should be treated as expression grouping, not function call syntax.

## Solution Implemented

Created `parsePostfixExpressionWithoutCall()` that parses field access and array subscripts but NOT function calls.
Use this in statement context (`parseIdentifierStatement()`).
...
### Known Limitation ❌
- ❌ `Text (x)*factor, y` - parenthesized expression with binary operator (window3d.bb pattern)
