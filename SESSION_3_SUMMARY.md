...
**Locations**: Offsets 0xcb30, 0xcb43, 0xcb53, 0xcb66, 0xd1a3, 0xd1b6, 0xd1c6, 0xd1d9, 
              0x17bee, 0x17c04, 0x17c14, 0x17c2a, 0x183d7, 0x18409, 0x185af, 0x18d5c
...
3. **Create minimal reproduction**
   Once pattern found, create test case
...
**After field initializers**: Still 86% (NPCs.bb partially fixed but not yet passing)
...
1. **Systematic investigation** - Quick checks identified field initializers immediately
2. **Minimal reproduction** - Simple test case reproduced the exact error
3. **Type-aware code generation** - Using `generateWithInfo()` instead of `generate()`
...
**Status**: Good progress on NPCs.bb. Field initializer bug was critical and affected multiple files (not just NPCs.bb). The remaining 16 local variable errors should be tractable once we identify the exact pattern.

**Ready for next session** - Complete NPCs.bb fix, then tackle DevilParticleSystem.bb for 92% pass rate milestone! 🚀
