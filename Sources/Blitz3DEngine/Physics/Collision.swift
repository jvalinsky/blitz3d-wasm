...
public struct AABB {
    ...
    @inline(__always)
    public init(min: Vec3 = Vec3(x: Float.greatestFiniteMagnitude), max: Vec3 = Vec3(x: -Float.greatestFiniteMagnitude)) {
        self.min = min
        self.max = max
    }
...
public class CollisionWorld {
    ...
    public func collideWithMeshes(id: Int32, meshId: Int32, surfaceIdx: Int32) -> CollisionResult {
        guard let collider = colliders[id],
              ...
              surfaceIdx < mesh.surfaces.count else {
    ...
    private func closestPointOnSegmentToPoint(segmentStart: Vec3, segmentEnd: Vec3, point: Vec3) -> Vec3 {
