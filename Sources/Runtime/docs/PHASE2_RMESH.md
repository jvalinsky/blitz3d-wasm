...
Phase 2 implements the RMesh parser for loading SCPCB room meshes and converting
them to Three.js geometry for browser rendering.
...
// Load a room
const entityId = await graphics.loadRMeshFile(
  "rooms/room1.rmesh",
  parentEntity,
);
```
...
When textures fail to load, a pink checkerboard pattern is used (matching
SCPCB's missing texture indicator).
