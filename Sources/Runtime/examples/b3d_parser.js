...
class B3DParser {
    ...
    parseBinaryData(data) {
        ...
        while (this.offset < this.data.length && chunksRead < maxChunks) {
            ...
            if (chunksRead <= 10) {
                console.log(`[B3DParser] Chunk: ${chunkName} (0x${chunkId.toString(16)}), size: ${chunkSize}`);
        ...
        console.log(`[B3DParser] Parsed: ${this.textures.length} textures, ${this.brushes.length} brushes, ${this.meshes.length} meshes`);
        return { textures: this.textures, brushes: this.brushes, meshes: this.meshes };
    }
    ...
    readTextures(chunkSize) {
        ...
        while (this.offset < endOffset) {
            ...
            this.textures.push({ name, flags, blend, position: { u: posU, v: posV }, scale: { u: scaleU, v: scaleV }, rotation });
        }
    ...
    readBrushes(chunkSize) {
        ...
        while (this.offset < endOffset) {
            ...
            this.brushes.push({ name, color: { r: red, g: green, b: blue }, alpha, shininess, blend, fx, textureIds: texIds });
        }
    ...
    readMesh(chunkSize) {
            ...
            switch (subChunkId) {
                case 0x56525453: // 'VRTS' (little-endian)
                    ...
                    while (this.offset < subEndOffset) {
                        ...
                        let normal = null;
                        if (vFlags & 1) {
                            normal = { x: this.readFloat32(), y: this.readFloat32(), z: this.readFloat32() };
                        }

                        let color = null;
                        if (vFlags & 2) {
                            color = { r: this.data[this.offset++], g: this.data[this.offset++], b: this.data[this.offset++], a: this.data[this.offset++] };
                        }
                ...
                case 0x54524953: // 'TRIS'
                    ...
                    while (this.offset < subEndOffset) {
                        triangles.push({ v0: this.readInt32LE(), v1: this.readInt32LE(), v2: this.readInt32LE() });
                    }
                    break;
        ...
        console.log(`[B3DParser] Mesh: ${vertices.length} verts, ${triangles.length} tris`);
    }
}
...
function createThreeJSFromB3D(b3dData, textureLoader) {
    ...
    for (const meshData of b3dData.meshes) {
        if (meshData.vertices.length === 0 || meshData.triangles.length === 0) continue;
        ...
        for (const tri of meshData.triangles) {
            if (tri.v0 < meshData.vertices.length && tri.v1 < meshData.vertices.length && tri.v2 < meshData.vertices.length) {
        ...
        let material;
        const brush = meshData.brushIndex >= 0 ? b3dData.brushes[meshData.brushIndex] : null;

        if (brush) {
            const texture = brush.textureIds && brush.textureIds[0] >= 0 && brush.textureIds[0] < b3dData.textures.length
                ? textureLoader(b3dData.textures[brush.textureIds[0]].name)
                : null;

            material = new window.THREE.MeshStandardMaterial({
                color: new window.THREE.Color(brush.color.r, brush.color.g, brush.color.b),
                opacity: brush.alpha,
            ...
            material = new window.THREE.MeshStandardMaterial({ color: 0x888888, side: window.THREE.DoubleSide });
        }
