...
class RMeshParserBrowser {
    ...
    clearTextureCache() {
        this.textureCache.forEach(t => { if (t?.image) t.dispose(); });
    ...
    async parseBinaryData(data, filePath) {
        ...
        for (let t = 0; t < textureCount; t++) {
            console.log(`[RMeshParser] Parsing texture ${t + 1}/${textureCount} at offset ${offset}`);
            ...
            console.log(`[RMeshParser] Lightmap flag: ${lightmapFlag}, path: "${lightmapPath}"`);
            ...
            console.log(`[RMeshParser] Texture flag: ${textureFlag}, path: "${texturePath}"`);
            ...
            if (vertexCount < 0 || vertexCount > 10000000) {
                console.error(`[RMeshParser] Invalid vertex count: ${vertexCount} at offset ${offset - 4}`);
            ...
            for (let v = 0; v < vertexCount; v++) {
                if (offset + 31 > data.length) {
                    console.error(`[RMeshParser] Unexpected end at vertex ${v}/${vertexCount}, offset ${offset}`);
                    break;
                }
            ...
            for (let tri = 0; tri < triangleCount; tri++) {
                if (offset + 12 > data.length) {
                    console.error(`[RMeshParser] Unexpected end at triangle ${tri}/${triangleCount}`);
                    break;
                }
    ...
    async parsePointEntity(data, offset) {
        ...
        switch (type) {
            ...
            case 'light':
                ...
                dataObj.color = { r: data[offset], g: data[offset + 1], b: data[offset + 2] };
                offset += 3;
    ...
    generatePlaceholderVertices(count) {
        ...
        for (let i = 0; i < count; i++) {
            vertices.push({
                ...
                uvs: [{ u: Math.random(), v: 1 - Math.random() }, { u: Math.random(), v: 1 - Math.random() }],
...
function createThreeJSMesh(meshData, textureLoader, isAlpha = false) {
    ...
    const texture = meshData.textures && meshData.textures[0] ? meshData.textures[0].texture : null;
    const lightmap = meshData.lightmaps && meshData.lightmaps[0] ? meshData.lightmaps[0].texture : null;
    
    // Debug: check vertex validity
    const firstVert = meshData.vertices[0];
    console.log('[RMeshParser] Mesh:', meshData.vertices.length, 'verts,', meshData.triangles.length, 'tris, texture:', texture ? 'yes' : 'no', 'lightmap:', lightmap ? 'yes' : 'no');
...
function createThreeJSFromRMesh(roomData, textureLoader) {
    const group = new window.THREE.Group();
    console.log('[RMeshParser] Creating room group, opaque:', roomData.opaque.length, 'alpha:', roomData.alpha.length);
