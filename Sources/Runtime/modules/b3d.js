...
class B3DLoader {
  ...
  async loadWithWasm(data, parentId) {
  ...
  async createThreeJSFromWasm(meshId, parentId) {
    const engine = window.Blitz3D.engineExports;
    ...
    for (let i = 0; i < surfCount; i++) {
      ...
      // We must CLONE the data if the WASM memory can grow/shift,
      // but for performance we try to use it directly if possible.
      // Three.js BufferAttribute can take the typed array.

      // Extract attributes from the interleaved buffer
      // (Three.js can also handle interleaved buffers via InterleavedBufferAttribute)
  ...
  parseBinaryData(data) {
    ...
    while (this.offset < this.data.length && chunksRead < maxChunks) {
      ...
      if (this.logCount < 5) {
        this.log(
          `[B3DLoader] Moving offset from ${this.offset} to ${newOffset}`,
        );
      }
    ...
    this.log(
      `[B3DLoader] Parsed: ${this.textures.length} textures, ${this.brushes.length} brushes, ${this.meshes.length} meshes, ${this.animations.length} animations`,
    );
  ...
  readBoneKeys(chunkSize) {
    ...
    while (
      this.offset < this.data.length && this.offset < this.offset + chunkSize
    ) {
    ...
    if (
      ...
      if (texIdx < b3dData.textures.length) {
        ...
        if (this.graphics.textureLoader) {
          texture = await this.graphics.textureLoader.loadTexture(
            texData.name,
            { flags: texData.flags },
          );
        }
