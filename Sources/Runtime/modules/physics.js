...
class Blitz3DPhysics {
  ...
  setupImports(imports) {
    ...
    imports.env.EntityBox = (entityId, x, y, z, width, height, depth) => {
      console.log(
        `EntityBox: entity=${entityId} pos=(${x},${y},${z}) size=(${width},${height},${depth})`,
      );
    ...
    imports.env.EntityRadius = (entityId, xRadius, yRadius) => {
      console.log(
        `EntityRadius: entity=${entityId} radius=(${xRadius},${yRadius})`,
      );
    ...
    imports.env.EntityRadius = (ent, r1, r2) => {
      ...
      if (entity) {
        ...
        if (
          this.useWasmCollision && this.wasmEngineReady &&
          entity.userData.colliderId
        ) {
          this.freeWasmCollider(entity.userData.colliderId);
          entity.userData.colliderId = this.createWasmCollider(
            r1,
            entity.userData.height,
          );
    ...
    imports.env.SetColliderPosition = (id, x, y, z) => {
      if (
        this.core.engineExports && this.core.engineExports.SetColliderPosition
      ) {
    ...
    imports.env.GetColliderPositionX = (id) => {
      if (
        this.core.engineExports && this.core.engineExports.GetColliderPositionX
      ) {
    ...
    imports.env.GetColliderPositionY = (id) => {
      if (
        this.core.engineExports && this.core.engineExports.GetColliderPositionY
      ) {
    ...
    imports.env.GetColliderPositionZ = (id) => {
      if (
        this.core.engineExports && this.core.engineExports.GetColliderPositionZ
      ) {
    ...
    imports.env.CollideWithMesh = (colliderId, meshId, surfaceIdx) => {
      if (this.core.engineExports && this.core.engineExports.CollideWithMesh) {
        return this.core.engineExports.CollideWithMesh(
          colliderId,
          meshId,
          surfaceIdx,
        );
    ...
    imports.env.CollisionDepth = (colliderId, meshId, surfaceIdx) => {
      if (this.core.engineExports && this.core.engineExports.CollisionDepth) {
        return this.core.engineExports.CollisionDepth(
          colliderId,
          meshId,
          surfaceIdx,
        );
    ...
    imports.env.CollisionNormalX = (colliderId, meshId, surfaceIdx) => {
      if (this.core.engineExports && this.core.engineExports.CollisionNormalX) {
        return this.core.engineExports.CollisionNormalX(
          colliderId,
          meshId,
          surfaceIdx,
        );
    ...
    imports.env.CollisionNormalY = (colliderId, meshId, surfaceIdx) => {
      if (this.core.engineExports && this.core.engineExports.CollisionNormalY) {
        return this.core.engineExports.CollisionNormalY(
          colliderId,
          meshId,
          surfaceIdx,
        );
    ...
    imports.env.CollisionNormalZ = (colliderId, meshId, surfaceIdx) => {
      if (this.core.engineExports && this.core.engineExports.CollisionNormalZ) {
        return this.core.engineExports.CollisionNormalZ(
          colliderId,
          meshId,
          surfaceIdx,
        );
    ...
    imports.env.AddSurface = (meshId, vertexCount, indexCount) => {
      if (this.core.engineExports && this.core.engineExports.AddSurface) {
        return this.core.engineExports.AddSurface(
          meshId,
          vertexCount,
          indexCount,
        );
    ...
    imports.env.GetMeshSurfaceCount = (meshId) => {
      if (
        this.core.engineExports && this.core.engineExports.GetMeshSurfaceCount
      ) {
    ...
    imports.env.GetSurfaceVertexCount = (meshId, surfaceIdx) => {
      if (
        this.core.engineExports && this.core.engineExports.GetSurfaceVertexCount
      ) {
        return this.core.engineExports.GetSurfaceVertexCount(
          meshId,
          surfaceIdx,
        );
      }
    ...
    imports.env.GetSurfaceIndexCount = (meshId, surfaceIdx) => {
      if (
        this.core.engineExports && this.core.engineExports.GetSurfaceIndexCount
      ) {
    ...
    imports.env.GetSurfaceVerticesPtr = (meshId, surfaceIdx) => {
      if (
        this.core.engineExports && this.core.engineExports.GetSurfaceVerticesPtr
      ) {
        return this.core.engineExports.GetSurfaceVerticesPtr(
          meshId,
          surfaceIdx,
        );
      }
    ...
    imports.env.GetSurfaceIndicesPtr = (meshId, surfaceIdx) => {
      if (
        this.core.engineExports && this.core.engineExports.GetSurfaceIndicesPtr
      ) {
    ...
    imports.env.LinePickDistance = (meshId, x, y, z, dx, dy, dz) => {
      if (this.core.engineExports && this.core.engineExports.LinePickDistance) {
        return this.core.engineExports.LinePickDistance(
          meshId,
          x,
          y,
          z,
  ...
  updateWasmCollisions() {
    ...
    for (const [entId, colliderId] of this.wasmColliderMap) {
      ...
      for (
        const [meshEntId, meshEntity] of Object.entries(this.graphics.entities)
      ) {
        ...
        for (let i = 0; i < meshEntity.userData.meshes.length; i++) {
          ...
          const surfaceCount =
            this.core.engineExports?.GetMeshSurfaceCount(meshId) || 0;

          for (let s = 0; s < surfaceCount; s++) {
            const collided = this.core.engineExports?.CollideWithMesh(
              colliderId,
              meshId,
              s,
            );
            if (collided) {
              const depth = this.core.engineExports?.CollisionDepth(
                colliderId,
                meshId,
                s,
              ) || 0;
              const nx = this.core.engineExports?.CollisionNormalX(
                colliderId,
                meshId,
                s,
              ) || 0;
              const ny = this.core.engineExports?.CollisionNormalY(
                colliderId,
                meshId,
                s,
              ) || 0;
              const nz = this.core.engineExports?.CollisionNormalZ(
                colliderId,
                meshId,
                s,
              ) || 0;
