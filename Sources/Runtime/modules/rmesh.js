...
class RMeshParser {
    ...
    async parseFile(filePath) {
        ...
        try {
            ...
            const triggerBoxes = hasTriggerBox ? this.parseTriggerBoxes(handle, basePath) : [];
            ...
            this.log(`  Opaque meshes: ${opaqueMeshes.length}, Alpha: ${alphaMeshes.length}`);
    ...
    async createThreeJSObjects(roomData) {
        ...
        for (const triggerData of roomData.triggerBoxes) {
            const mesh = await this.createThreeJSMesh(triggerData.mesh, roomData.basePath);
            if (mesh) {
    ...
    async createThreeJSMesh(meshData, basePath) {
        ...
        // Load textures
        const materials = await this.createMaterials(meshData.textures, basePath, meshData.isAlpha);
