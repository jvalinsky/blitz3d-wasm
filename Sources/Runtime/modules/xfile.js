...
class XFileLoader {
    ...
    parseText(text) {
        ...
        this.log(`[XFileLoader] Starting parse loop, tokens.length: ${tokens.length}`);

        while (index < tokens.length) {
            this.log(`[XFileLoader] Loop iteration: index=${index}, token="${tokens[index]}"`);
    ...
    parseTransformMatrix(tokens, index, target) {
        ...
        if (tokens[index] !== '{') {
            this.log(`[XFileLoader] parseTransformMatrix: expected '{' but got "${tokens[index]}"`);
            return index;
        }
        index++;
        ...
        this.log(`[XFileLoader] parseTransformMatrix: read ${values.length} values in ${readCount} iterations`);
    ...
    parseMesh(tokens, index, result, parentFrame = null) {
        ...
        while (vertexRead < mesh.vertexCount && index < tokens.length) {
            ...
            if (!isNaN(parseFloat(tokens[index]))) {
                ...
                while (index < tokens.length && (tokens[index] === ';' || tokens[index] === ',')) index++;
            } else {
                index++;
            }
        }
        ...
        this.log(`[XFileLoader] Parse complete. Final index: ${index}, tokens.length: ${tokens.length}`);
