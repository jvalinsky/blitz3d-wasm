...
class ParticleSystem {
    ...
    initShaders() {
        ...
        this.locations = {
            ...
            a_instancePosition: gl.getAttribLocation(this.program, 'a_instancePosition'),
            ...
            a_instanceRotation: gl.getAttribLocation(this.program, 'a_instanceRotation'),
    ...
    UpdateParticles(deltaTime) {
        ...
        for (const [emitterId, emitter] of this.emitters) {
            ...
            if (emitter.template.emitterLife > 0 && emitter.age > emitter.template.emitterLife) {
        ...
        for (const [id, particle] of this.particles) {
            ...
            if (particle.floorY !== -Infinity && particle.position[1] < particle.floorY) {
    ...
    _createParticle(emitter) {
        ...
        const lifeTime = t.particleLifeMin + Math.random() * (t.particleLifeMax - t.particleLifeMin);
        ...
        return {
            ...
            velocity: [
                t.velocityRange.x[0] + Math.random() * (t.velocityRange.x[1] - t.velocityRange.x[0]),
                t.velocityRange.y[0] + Math.random() * (t.velocityRange.y[1] - t.velocityRange.y[0]),
                t.velocityRange.z[0] + Math.random() * (t.velocityRange.z[1] - t.velocityRange.z[0])
            ],
            lifeTime: lifeTime,
            age: 0,
            size: t.sizeRange.min + Math.random() * (t.sizeRange.max - t.sizeRange.min),
            sizeVel: t.sizeVelRange.min + Math.random() * (t.sizeVelRange.max - t.sizeVelRange.min),
            rotation: t.rotationRange.min + Math.random() * (t.rotationRange.max - t.rotationRange.min),
            rotationVel: 0,
    ...
    render(projectionMatrix, viewMatrix) {
        ...
        for (const [id, particle] of this.particles) {
            ...
            const alpha = particle.alphaStart + (particle.alphaEnd - particle.alphaStart) * lifeRatio;
            ...
            instanceData[offset++] = color.r * particle.colors[0].r * particle.colors[0].r * this.templates.get(1)?.brightness || color.r;
            instanceData[offset++] = color.g * particle.colors[0].g * particle.colors[0].g;
            instanceData[offset++] = color.b * particle.colors[0].b * particle.colors[0].b;
        ...
        const blendMode = this.templates.size > 0 ? this.templates.values().next().value.blendMode : 0;
