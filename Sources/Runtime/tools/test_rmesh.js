    ...
    try {
      ...
      for (const test of testCases) {
        ...
        if (isValid === test.isValid && hasTriggerBox === test.hasTriggerBox) {
          ...
          throw new Error(
            `Header "${test.header}" detection failed: valid=${isValid} expected=${test.isValid}`,
          );
        }
      }
    } catch (error) {
  ...
  async testEntityParsing() {
    ...
    try {
      ...
      const testEntities = [
        ...
        {
          type: "soundemitter",
          data: { soundFile: "test.wav", loop: true, vol: 0.5 },
        },
    ...
    try {
      ...
      for (const test of testCases) {
        ...
        if (
          ...
          throw new Error(
            `Coordinate conversion failed for ${JSON.stringify(test.input)}`,
          );
        }
      }
  ...
  async testCoordinateConversion() {
    ...
    try {
      ...
      for (const test of testColors) {
        ...
        if (
          Math.abs(normalized[0] - test.expected[0]) < 0.001 &&
          ...
          throw new Error(
            `Color conversion failed for RGB(${test.r}, ${test.g}, ${test.b})`,
          );
        }
      }
  ...
  async testRMeshFileDetection() {
    ...
    try {
      ...
      if (fs.existsSync(mapPath)) {
        const files = fs.readdirSync(mapPath).filter((f) =>
          f.endsWith(".rmesh")
        );
        ...
        console.log(
          "  ⚠ SCPCB assets not found (expected in development environment)",
        );
        this.passed++;
      }
    } catch (error) {
