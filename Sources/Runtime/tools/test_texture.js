...
class TextureTest {
    ...
    async testTextureCaching() {
        ...
        try {
            ...
            if (hitCount === textures.length) {
                console.log(`  ✓ Cache retrieval works (${hitCount}/${textures.length})`);
                this.passed++;
            } else {
                throw new Error(`Cache retrieval failed: ${hitCount}/${textures.length}`);
            }
    ...
    async testPlaceholderCreation() {
        ...
        try {
            ...
            for (const test of placeholderColors) {
                ...
                const isLightPink = (Math.floor(test.x / 32) + Math.floor(test.y / 32)) % 2 === 0;
    ...
    async testMaterialSystem() {
        ...
        try {
            ...
            if (createdBrush.textures[0] && createdBrush.textures[0].path === 'test.png') {
