...
const Blitz3D = {
    ...
    loadAsset: function (path) {
        ...
        if (this.assetManifest && this.assetManifest.embedAssets) {
            ...
            if (assetInfo && assetInfo.offset) {
                ...
                const buffer = data.slice(assetInfo.offset, assetInfo.offset + assetInfo.size);
                const asset = { data: buffer, embedded: true, offset: assetInfo.offset };
    ...
    loadAssetManifest: function (manifestUrl) {
        ...
        return fetch(manifestUrl)
            ...
            .then(manifest => {
                ...
                console.log("Loaded asset manifest with " + (manifest.assets?.length || 0) + " assets");
                console.log("Embedded assets: " + (manifest.embedAssets ? "yes" : "no"));
            })
            .catch(err => {
    ...
    imports: {
        env: {
            ...
            ClsColor: function (r, g, b) {
                ...
                if (Blitz3D.renderer) {
                    Blitz3D.renderer.setClearColor(new THREE.Color(r / 255.0, g / 255.0, b / 255.0), 1.0);
                }
            },
            ...
            Cls: function () {
                ...
                if (Blitz3D.ctx2d) {
                    Blitz3D.ctx2d.clearRect(0, 0, Blitz3D.textCanvas.width, Blitz3D.textCanvas.height);
                }
            },

            // Entity Management
            CreateCamera: function (parent) {
                const cam = new THREE.PerspectiveCamera(75, Blitz3D.canvas.width / Blitz3D.canvas.height, 1.0, 1000.0);
            ...
            CreateLight: function (type, parent) {
                ...
                switch (type) {
                    ...
                    case 2: light = new THREE.PointLight(0xffffff, 1, 100); break;
                    case 3: light = new THREE.SpotLight(0xffffff, 1, 100, Math.PI / 4); break;
                    default: light = new THREE.DirectionalLight(0xffffff, 1);
                }
                const id = Blitz3D.nextEntityId++;
            ...
            AmbientLight: function (r, g, b) {
                if (!Blitz3D.ambientLight) {
                    Blitz3D.ambientLight = new THREE.AmbientLight(new THREE.Color(r / 255, g / 255, b / 255), 1.0);
            ...
            CameraClsColor: function (camId, r, g, b) {
                ...
                Blitz3D.renderer.setClearColor(new THREE.Color(r / 255, g / 255, b / 255), 1);
            },
            ...
            Animate: function (entId, mode, speed, seq, transition) {
                ...
                if (obj && obj.blitzMixer && obj.blitzAnimations[seq]) {
                    ...
                    switch (mode) {
                        case 1: action.setLoop(THREE.LoopRepeat); break;
                        case 2: action.setLoop(THREE.LoopPingPong); break;
                        case 3: action.setLoop(THREE.LoopOnce); break;
                    }
            ...
            Animating: function (entId) {
                const obj = Blitz3D.entities[entId];
                return (obj && obj.blitzActiveAction && obj.blitzActiveAction.isRunning()) ? 1 : 0;
            },
            ...
            EntityVisible: function (srcId, destId) {
                ...
                const intersects = raycaster.intersectObjects(Blitz3D.scene.children, true);
            ...
            EntityInView: function (entId, camId) {
                ...
                if (ent.isMesh) {
                    if (!ent.geometry.boundingSphere) ent.geometry.computeBoundingSphere();
                    const sphere = ent.geometry.boundingSphere.clone().applyMatrix4(ent.matrixWorld);
            ...
            ReadData: function (stream, bankHandle, length) {
                ...
                if (file && bank) {
                    ...
                    const fileBytes = new Uint8Array(file.buffer.buffer, file.buffer.byteOffset + file.pos, count);
                    const bankBytes = new Uint8Array(Blitz3D.memory.buffer, bank.ptr, bank.size);
            ...
            UpdateWorld: function (elapsed) {
                ...
                rules.forEach(rule => {
                    const sources = entities.filter(e => e.blitzType === rule.src && e.blitzRadius);
                        ...
                        targets.forEach(dest => {
                            ...
                            if (rule.method === 1 && dest.blitzRadius) {
                                ...
                                if (dist < minDist) {
                                    ...
                                    const dir = new THREE.Vector3().subVectors(src.position, dest.position).normalize();
                            ...
                            if (rule.method === 2 && dest.isMesh) {
                                ...
                                const srcSphere = new THREE.Sphere(src.position, src.blitzRadius.x);
                                if (!dest.geometry.boundingBox) dest.geometry.computeBoundingBox();
                                const box = dest.geometry.boundingBox.clone().applyMatrix4(dest.matrixWorld);

                                if (box.intersectsSphere(srcSphere)) {
                                    // In a real engine, we'd do per-triangle. 
                                    // For now, let's just push back from the box center.
                                    const dir = new THREE.Vector3().subVectors(src.position, box.getCenter(new THREE.Vector3())).normalize();
            ...
            CollisionX: function (ent, idx) { return Blitz3D.physicsManager.lastPick ? Blitz3D.physicsManager.lastPick.point.x : 0; },
            CollisionY: function (ent, idx) { return Blitz3D.physicsManager.lastPick ? Blitz3D.physicsManager.lastPick.point.y : 0; },
            CollisionZ: function (ent, idx) { return Blitz3D.physicsManager.lastPick ? -Blitz3D.physicsManager.lastPick.point.z : 0; },
            ...
            CountCollisions: function (ent) { return 0; },
            CollisionNX: function (ent, idx) { return 0; },
            CollisionNY: function (ent, idx) { return 0; },
            CollisionNZ: function (ent, idx) { return 0; },
            CollisionSurface: function (ent, idx) { return 0; },
            CollisionTriangle: function (ent, idx) { return 0; },
            ...
            Cls: function () {
                ...
                if (Blitz3D.ctx2d) {
                    Blitz3D.ctx2d.clearRect(0, 0, Blitz3D.canvas.width, Blitz3D.canvas.height);
                }
            },
            ...
            Oval: function (x, y, w, h, solid) {
                if (Blitz3D.ctx2d) {
                    ...
                    Blitz3D.ctx2d.ellipse(x + w / 2, y + h / 2, w / 2, h / 2, 0, 0, 2 * Math.PI);
            ...
            Text: function (x, y, strPtr, centerX, centerY) {
                if (Blitz3D.ctx2d) {
                    ...
                    Blitz3D.ctx2d.font = `${Blitz3D.currentFontSize}px ${Blitz3D.currentFont}`;
            ...
            TileImage: function (imgId, x, y, frame) {
                ...
                if (meta && meta.loaded && Blitz3D.ctx2d) {
                    ...
                    Blitz3D.ctx2d.fillRect(0, 0, Blitz3D.canvas.width, Blitz3D.canvas.height);
                    // Restore color
                    Blitz3D.ctx2d.fillStyle = `rgb(${Blitz3D.currentColor[0]},${Blitz3D.currentColor[1]},${Blitz3D.currentColor[2]})`;
                }
            },
            ...
            MouseX: function () { return Blitz3D.mouseX; },
            MouseY: function () { return Blitz3D.mouseY; },
            MouseZ: function () { return Blitz3D.mouseZ; },
            MouseDown: function (btn) { return Blitz3D.mouseButtons[btn] ? 1 : 0; },
            ...
            Sgn: function (n) { return Math.sign(n); },
            ...
            LoadSound: function (filePtr) {
                ...
                fetch(url)
                    .then(response => response.arrayBuffer())
                    .then(arrayBuffer => Blitz3D.audioContext.decodeAudioData(arrayBuffer))
                    .then(audioBuffer => {
            ...
            PlaySound: function (soundId) {
                if (Blitz3D.sounds[soundId] && Blitz3D.sounds[soundId].loaded && Blitz3D.audioContext) {
            ...
            CopyBank: function (srcHandle, srcOffset, destHandle, destOffset, count) {
                ...
                if (src && dest) {
                    const mem = new Uint8Array(Blitz3D.memory.buffer);
                    mem.set(mem.subarray(src.ptr + srcOffset, src.ptr + srcOffset + count), dest.ptr + destOffset);
                }
            },
            ...
            FSOUND_Init: function (freq, maxchannels, flags) {
                console.log(`FSOUND_Init: ${freq}Hz, ${maxchannels} chans, flags=${flags}`);
                Blitz3D.initAudio();
            ...
            FSOUND_Stream_Open: function (pathPtr, mode, offset, length) {
                ...
                if (data) {
                    ...
                    if (ctx) {
                        ...
                        const bufferCopy = data instanceof ArrayBuffer ? data.slice(0) : data.buffer.slice(data.byteOffset, data.byteOffset + data.byteLength);
                        ctx.decodeAudioData(bufferCopy,
                            (audioBuffer) => {
    ...
    load: async function (wasmUrl, canvasId) {
        if (canvasId) {
            ...
            console.error("Blitz3D canvas not initialized. Call Blitz3D.init(id) first or pass id to load().");
            return;
        }
        this.setupInput();
