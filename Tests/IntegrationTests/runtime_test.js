  ...
  for (const test of tests) {
    ...
    try {
      ...
      for (const expected of test.expectedOutput) {
        const found = runtime.consoleOutput.some((o) =>
          o.includes(expected.split(":")[1].trim())
        );
        if (!found) {
          outputMatch = false;
          break;
...
`;
  ...
  if (fs.existsSync(testWasmPath)) {
    ...
    console.log(
      "  Generate one with: swift run blitz3d-wasm test.bb -o test.wasm",
    );
  }
}
