...
console.log('Test 4: Blitz3DAnimation can be instantiated with mock dependencies...');
try {
    const { Blitz3DAnimation } = require(path.join(modulesDir, 'animation'));
...
try {
    ...
    const mockGraphics = { nextEntityId: 1, entities: {}, scene: null, textureLoader: null };
    const mockCore = {
        fileIO: {
            openFile: () => 1,
            closeFile: () => {},
    ...
    const minimalB3D = new Uint8Array([
        ...
        0, 0, 0, 0       // Empty TEXS chunk (0 bytes)
    ]);
