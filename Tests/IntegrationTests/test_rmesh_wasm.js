...
async function runWasm() {
    ...
    const readString = (ptr) => {
        ...
        const bytes = new Uint8Array(memory.buffer);
        const length = bytes[ptr + 4] | (bytes[ptr + 5] << 8) | (bytes[ptr + 6] << 16) | (bytes[ptr + 7] << 24);
    ...
    const envBase = {
        ...
        ReadFile: (pathPtr) => {
            ...
            try {
                ...
                console.log(`[ReadFile] Opened: fileId=${fileId}, size=${fileData.length}`);
                return fileId;
            } catch (err) {
        ...
        ReadString: (fileId) => {
            ...
            file.offset += 4;
            if (len < 0 || file.offset + len > file.data.length) return allocString("");
            let str = "";
            for (let i = 0; i < len; i++) {
        ...
        StringConcat: (ptr1, ptr2) => allocString(readString(ptr1) + readString(ptr2)),
        ...
        Replace: (str, oldStr, newStr) => allocString(readString(str).split(readString(oldStr)).join(readString(newStr))),
        Rnd: () => Math.random(),
    ...
    const env = new Proxy(envBase, {
        get: (target, prop) => {
            ...
            if (typeof prop === 'string' && /^[a-zA-Z]/.test(prop)) {
                return () => {
                    if (prop.toLowerCase().includes('create') || prop.toLowerCase().includes('load') || prop.toLowerCase().includes('get')) {
                        return 0;
                    }
                    return 0;
                };
    ...
    try {
        ...
        memory = instance.exports.memory || instance.exports.mem || instance.exports.Memory;
        ...
        const mainFunc = instance.exports._start || instance.exports.Main || instance.exports.main || instance.exports.bb_Main;
