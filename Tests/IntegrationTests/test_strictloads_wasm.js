...
async function runWasm() {
  ...
  const readString = (ptr) => {
    ...
    if (ptr < 1000) {
      console.log(
        `[readString] ptr=${ptr}, first 20 bytes: ${
          Array.from(bytes.slice(ptr, ptr + 20)).join(", ")
        }`,
      );
    }
    ...
    if (length > 10000 || length < 0) {
      console.log(
        `[readString] Invalid length ${length} at ptr ${ptr}, trying null-terminated`,
      );
      let str = "";
  ...
  const env = {
    ...
    LoadMesh: (pathPtr, parent) => {
      ...
      try {
        ...
        console.log(
          `[LoadMesh] Created mesh ${meshId} with ${meshData.surfaces.length} surface(s)`,
        );
        return meshId;
      } catch (err) {
  ...
  const createProxy = (target, moduleName) =>
    new Proxy(target, {
      get: (t, prop) => {
        if (prop in t) {
          return t[prop];
        ...
        return (...args) => {
          console.log(`[MISSING ${moduleName}] ${prop}(${
            args.map((a) => typeof a === "number" ? a : "?").join(", ")
          })`);
          return 0;
        };
      },
  ...
  try {
    ...
    if (!memory) {
      ...
      console.log(
        "Available exports:",
        Object.keys(instance.exports).filter((k) => !k.startsWith("_")).slice(
          0,
          20,
        ),
      );
