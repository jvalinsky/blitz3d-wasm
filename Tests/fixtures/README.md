...
This directory contains minimal test cases for validating compiler behavior.
Each test is a standalone `.bb` file that can be compiled and validated
independently.
...
- **test_suffix_same_var.bb** - Verify `x` and `x#` refer to same variable
  (Blitz3D semantics)
...
Tests created during debugging sessions to reproduce specific bugs. These serve
as regression tests to prevent bugs from reappearing.
...
- **Problem**: Variables with type suffixes (`#`, `%`, `$`) were not preserving
  type information
...
- All tests are minimal - they should be as small as possible while still
  reproducing the issue
- Tests should be self-contained (no includes or dependencies)
- Tests should either compile successfully or fail with a specific expected
  error
