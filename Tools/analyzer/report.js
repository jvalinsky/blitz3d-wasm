...
`;
  ...
  generateSummarySection(summary, metrics) {
    ...
    section += `  ✓ Stack Balance:       ${summary.stackValid ? 'PASS' : 'FAIL'}\n`;
    section += `  ✓ Type Consistency:    ${summary.typeValid ? 'PASS' : 'FAIL'}\n`;
    section += `  ✓ Control Flow:        ${summary.controlFlowValid ? 'PASS' : 'FAIL'}\n\n`;
  ...
  generateStackBalanceSection() {
    ...
    if (stack.errors?.length > 0) {
      ...
      // Group by type
      const typeErrors = stack.errors.filter(e => e.toLowerCase().includes('type'));
      const stackErrors = stack.errors.filter(e => e.toLowerCase().includes('stack'));
      const otherErrors = stack.errors.filter(e => !typeErrors.includes(e) && !stackErrors.includes(e));
    ...
    if (stack.functionResults?.length > 0) {
      ...
      if (failing.length > 0) {
        failing.forEach(f => {
          section += `  ✗ Function ${f.funcIdx}: ${f.errors.length} error(s), max stack: ${f.maxStack}\n`;
        });
      } else {
  ...
  generateTypeConsistencySection() {
    ...
    if (types.issues?.length > 0) {
      ...
      if (errors.length > 0) {
        ...
        errors.forEach(e => {
          section += `  ✗ Function ${e.funcIdx}, Local ${e.localIdx}: ${e.message}\n`;
        });
        section += '\n';
      }
  ...
  generateControlFlowSection() {
    ...
    if (cf.results?.length > 0) {
      ...
      const invalidBranches = cf.results.filter(r => r.type === 'invalid_branch_depth');
      const unbalancedBlocks = cf.results.filter(r => r.type === 'unbalanced_blocks');

      if (invalidBranches.length > 0) {
        section += `Invalid Branch Depths (${invalidBranches.length}):\n`;
        invalidBranches.forEach(b => {
          section += `  ✗ Function ${b.funcIdx}, instruction ${b.idx}: ${b.message}\n`;
        });
        section += '\n';
      }
  ...
  generateMetricsSection() {
    ...
    if (stackDepths.length > 0) {
      ...
      const avgStack = stackDepths.reduce((sum, d) => sum + d.max, 0) / stackDepths.length;
    ...
    section += `  • Avg per Func: ${locals.length > 0 ? (locals.reduce((a, b) => a + b, 0) / locals.length).toFixed(1) : 0}\n`;
    section += '\n';
  ...
  generateSuggestionsSection() {
    ...
    if (!this.analysis.summary.stackValid) {
      suggestions.push({
        ...
        suggestion: 'Review if/else branches for consistent stack effects. Ensure function calls with return values are properly handled (drop the return value if unused).'
      });
    }
    ...
    if (!this.analysis.summary.typeValid) {
      suggestions.push({
        ...
        suggestion: 'Check function call argument ordering. Ensure numeric literals have correct types. Review type conversion operations.'
      });
    }
    ...
    if (!this.analysis.summary.controlFlowValid) {
      suggestions.push({
        ...
        suggestion: 'Verify branch depths are correct. Ensure all blocks (if/loop/block) are properly closed with "end" instructions.'
      });
    }
    ...
    if (metrics?.stackDepths?.some(d => d.max > 100)) {
      suggestions.push({
        ...
        suggestion: 'Consider breaking complex expressions into smaller parts or using local variables to reduce stack pressure.'
      });
    }
  ...
  generateJUnitXML() {
    ...
    const types = this.analysis.typeConsistency;
    const typeFailures = types?.issues?.filter(i => i.severity === 'error').length || 0;
    testsuites.push({
    ...
    xml += `<testsuites name="WASM Analysis" tests="${testCount}" failures="${failures}">\n`;

    testsuites.forEach(ts => {
      xml += `  <testsuite name="${ts.name}" tests="${ts.tests}" failures="${ts.failures}">\n`;
