...
const run = async (cmd: string[]) => {
    const p = new Deno.Command(cmd[0]!, { args: cmd.slice(1), stdout: "null", stderr: "piped" }).spawn();
...
const hasFfmpeg = async () => {
    try {
        const p = new Deno.Command("ffmpeg", { args: ["-version"], stdout: "null", stderr: "null" }).spawn();
...
const main = async () => {
    ...
    for (const p of wavFiles) {
        ...
        try {
            ...
            await run(["ffmpeg", "-y", "-i", p, "-vn", "-map_metadata", "-1", oggPath]);
            converted++;
            if (args.deleteSource) await Deno.remove(p);
        } catch (e) {
