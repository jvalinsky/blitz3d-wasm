...
const parseArgs = (): Args => {
  if (
    Deno.args.includes("--check-tools") || Deno.args.includes("-h") ||
...
const main = async () => {
  ...
  console.log(
    `[textures] Scanning ${args.root} (Target: ${args.format.toUpperCase()})...`,
  );
...
const convertToKtx2 = async (args: Args) => {
  ...
  console.log(
    `[textures] Found ${files.length} candidate files for KTX2 conversion`,
  );
  if (files.length === 0) return;
