...
Deno.test("scpcb_audit_gate: passes when no new literals and no dynamic calls", async () => {
  ...
  const baseline = {
    summary: {
      root: scpcb,
      totals: { assetHits: 1, dynamicMeshCalls: 0 },
      extensions: {
        b3d: { hits: 0, unique: 0 },
        x: { hits: 1, unique: 1 },
        rmesh: { hits: 0, unique: 0 },
      },
    },
    assets: [{
      file: "Main.bb",
...
Deno.test("scpcb_audit_gate: fails on dynamic LoadMesh call unless allow-dynamic", async () => {
  ...
  const baseline = {
    summary: {
      root: scpcb,
      totals: { assetHits: 0, dynamicMeshCalls: 0 },
      extensions: {
        b3d: { hits: 0, unique: 0 },
        x: { hits: 0, unique: 0 },
        rmesh: { hits: 0, unique: 0 },
