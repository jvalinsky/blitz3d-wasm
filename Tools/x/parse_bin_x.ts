...
class BinaryParser {
  ...
  private eatToken(token: number) {
    ...
    if (t !== token) {
      throw new Error(
        `BinX: Expected token ${token}, got ${t} at ${this.pos - 2}`,
      );
    }
  }
  ...
  private readInteger(): number {
    if (this.peekToken() === T.TOKEN_INTEGER_LIST) {
      ...
      if (count !== 1) {
        throw new Error(`BinX: Expected scalar integer, got list of ${count}`);
      }
  ...
  private readFloat(): number {
    if (this.peekToken() === T.TOKEN_FLOAT_LIST) {
      ...
      if (count !== 1) {
        console.warn(
          `BinX: Expected scalar float, got list of ${count}, using first`,
        );
      }
  ...
  private parseFrameBody(name?: string): XFrame {
    ...
    while (
      this.peekToken() !== T.TOKEN_CBRACE && this.pos < this.view.byteLength
    ) {
      if (this.peekToken() === T.TOKEN_NAME) {
  ...
  private parseMeshBody(name?: string): XMesh {
    ...
    if (this.peekToken() === T.TOKEN_INTEGER_LIST) {
      ...
      while (ptr < list.length) {
        ...
        if (ptr + nIndices > list.length) {
          throw new Error("BinX: Face list overflow");
        }
      ...
      for (let i = 0; i < nFaces; i++) {
        ...
        if (this.peekToken() === T.TOKEN_NAME) {
          ...
          if (
            ...
            console.warn(
              `BinX: Hit ${name} inside faces loop at ${i}/${nFaces}. Breaking.`,
            );
            this.pos = saved;
            break;
          }
        ...
        if (this.peekToken() === T.TOKEN_INTEGER_LIST) {
          const list = this.readIntegerList();
          if (list.length !== nIndices) {
            throw new Error("Face indices mismatch");
          }
          // Triangulate
