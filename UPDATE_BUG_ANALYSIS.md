...
So the issue is that **some** globals are being assigned f32 values when they should be i32.
...
All of these default to i32 (correct), but something is assigning f32 values to them.
...
Start with Update.bb and remove code until error disappears, to isolate exact pattern.
...
For complex expressions like `Int Sector$(...)`, log what type the whole expression returns.
...
## Status
**BLOCKED** - Need more targeted debugging. The issue is subtle and requires tracing through a specific assignment to see where the wrong type comes from.
