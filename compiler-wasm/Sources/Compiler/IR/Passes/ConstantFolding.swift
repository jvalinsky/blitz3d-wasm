...
import Foundation
    ...
    import Glibc
#elseif arch(wasm32) && canImport(WASI)
    import WASI
#endif
        ...
        case .ifStmt(let cond, let thenBranch, let elseBranch):
            ...
            if case .constI32(let val) = optimizedCond {
                if val != 0 {
                    ...
                    return thenBranch.isEmpty
                        ? .nop : .block(label: "", body: optimizeEffects(thenBranch))
                } else {
                    // Condition is always false - return else branch only
                    if let elseBranch = elseBranch {
                        return elseBranch.isEmpty
                            ? .nop : .block(label: "", body: optimizeEffects(elseBranch))
                    } else {
                        return .nop
                    }
                }
