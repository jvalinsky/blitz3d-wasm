...
See [DOCS_CATALOG.md](DOCS_CATALOG.md) for a curated map of docs across the repo.
...
- Documented the BB→WASM smoke suite (`Tests/deno_smoke/` + `Tools/tests/bb_deno_compile_and_run_smoke.test.ts`).
- Documented the “safe runner” approach (Worker + watchdog timeout) used by `web/interpreter.html` and `web/public/bb_wasm_runner_demo.html`.
