...
The video system bridges Blitz3D's `BlitzMovie_*` API to HTML5 `<video>` elements, allowing SCPCB startup videos to play in the browser.
...
2. **Async playback** - `Play()` returns immediately (browser autoplay restrictions)
3. **No AVI on Safari** - Safari doesn't support AVI codec (workaround: transcode to MP4)
