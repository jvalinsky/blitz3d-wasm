...
1. **Function index → name mapping**
   ...
   - Outcome: when validate says `func 549`, we immediately know which Blitz
     function it is.
...
2. **Emitter-side typed stack simulator (debug-only)**
   ...
   - Outcome: mismatch points to the exact opcode that introduced/consumed the
     wrong type.

## P1 — Likely sources of `expected i32, found f32`

1. **Index coercions in all array access paths**
   - Ensure indices are always `i32` at the point they reach `load/store` or
     runtime helpers.
   - Pay attention to: multi-dim flattening, computed indices, and mixed numeric
     expressions.
...
3. **Drop/discard correctness**
   ...
   - Any statement that must _preserve_ a value must not be over-dropped.
...
2. **Differential tests**
   - Compile same snippet via “old path” and “IR path” and compare key runtime
     behavior.
