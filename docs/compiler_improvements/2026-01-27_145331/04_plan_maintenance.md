...
- `docs/compiler_improvements/AGENTs.md` should list the current plan set and
  `archive/` as children.
- If folder names change, update `INDEX.md` + `AGENTs.md` together.

## Build warnings about AGENTs.md

SwiftPM currently warns about `AGENTs.md` files that live under `Sources/` and
`Tests/`. These are not part of this docs plan set, but they are worth cleaning
up because they add noise to test output.
