...
- A particle or sprite demo runs at interactive framerate without heavy boundary
  overhead.
...
- Legacy BB code often uses tight loops that will freeze the browser if called
  directly.
...
- A “known init path” never blocks on missing files because required assets are
  preloaded.
