...
4. Provide a **source-linked map**:
   - “Here’s the concept” → “Here’s where it lives in this repo” → “Here’s the external reference”.

## Audience

- “Future us” (or a new contributor) rebuilding the same system with a clean repo.
- Someone who wants to port another large Blitz3D game, not only SCPCB.

## Non-Goals

- Re-documenting every file in `docs/` (that already exists).
- Copying existing docs verbatim; this folder is an **integration + reconstruction** narrative.
...
- Docs: `docs/ARCHITECTURE.md`, `docs/COMPILER_DESIGN.md`, `docs/COMMAND_BUFFER_SYSTEM.md`

### Step 2: Extract “thin vertical slices”

Update `00_rebuild_roadmap.md` with slices that always end in a runnable artifact:
...
- Explicit “forbidden patterns” (e.g. tight infinite loops in WASM called from UI thread).
...
- Which asset formats should be “runtime supports” vs “offline conversion required”?
...
- “Gotchas” list derived from this repo (arrays, strings, stack balancing, includes, suffix typing).
