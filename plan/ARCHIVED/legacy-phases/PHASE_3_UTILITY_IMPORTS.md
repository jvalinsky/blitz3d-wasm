...
- [ ] **CodeGenerator:** Add imports for:
  ...
  - `Hex$(value)`
  - `Bin$(value)`
- [ ] **Runtime (JS):** Implement using standard JavaScript `charCodeAt`,
      `String.fromCharCode`, and `toString(16/2)`.
...
- [ ] **Runtime (JS):** Update event listeners to track delta movement and key
      buffers.

## Verification

- Compile and validate `Blitz_Basic_Bank.bb`.
- Ensure `validate_compiler.sh` passes for all math and string utility files in
  `scpcb/`.
