...
Run SCP: Containment Breach fully in the browser without crashing, using async
asset loading, streaming, and stable memory behavior.
...
- If progress stalls at 0%: ensure `Content-Length` and fetch streaming work (no
  opaque responses).
...
- If memory spikes: confirm chunk boundaries; ensure large textures not in
  boot/core.
...
- If game logic assumes immediate availability: add warnings when assets
  accessed early.
...
**Goal:** Identify and implement missing Blitz3D runtime imports required by
SCPCB.
...
2. **Prioritize missing APIs**
   - Critical path: collision, RMesh loader, audio spatialization, entity
     parenting, animation, input.
...
- If missing props in new room: verify preload triggers and manifest group
  mapping.
