...
This document outlines the technical plan for deploying SCP - Containment Breach
(SCPCB) as a WebAssembly application running in web browsers. The original game,
written in BlitzBasic (~52,000 lines across 35 .bb files), will be compiled to
WASM using the blitz3d-wasm compiler infrastructure and served with optimized
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("textures")) {
          const textureStore = db.createObjectStore("textures", {
            keyPath: "id",
          });
