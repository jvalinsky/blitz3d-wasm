...
Load and execute our BB→WASM output using **WasmKit** from the native host,
including:
...
- [ ] Ensure missing imports fail loudly with a list of missing symbols (no
      silent stubs by default).
...
- [ ] Decide and document the canonical string ABI for native host:
  - either “Blitz string object layout” (recommended if matching existing
    runtime),
...
- [ ] Decide how to enforce responsiveness:
  ...
  - run WASM tick on a worker thread and communicate back (harder on macOS UI
    thread; still feasible).
- [ ] Implement a “Stop” control / kill switch for runaway loops (debug builds
      at minimum).
