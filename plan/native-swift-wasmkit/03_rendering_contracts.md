...
Define and implement the rendering bridge between WASM and the native Metal renderer.
...
- Pixel format:
  - pick one: BGRA8 (preferred for Metal) or RGBA8 (more common in WASM examples)
...
- [ ] Each frame:
  ...
  - copy bytes from WASM memory into texture (`replaceRegion` or a staging buffer),
...
- Bridge errors are diagnosable (bad pointer/size/ABI -> clear log + safe abort).
