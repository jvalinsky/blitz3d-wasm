...
This doc collects external references and practical options for implementing the native macOS host described in this plan set.
...
References:
- Apple docs: `MTKView` — `https://developer.apple.com/documentation/metalkit/mtkview`
- Apple docs: `MTKViewDelegate` — `https://developer.apple.com/documentation/metalkit/mtkviewdelegate`

### Frame pacing / watchdog considerations

- `MTKView` can drive the loop automatically; you can also control pacing via `preferredFramesPerSecond`.
- For freeze resistance, treat the WASM “tick” as a **budgeted** call:
  - record start time,
  - if elapsed > threshold, trigger a stop condition (in debug builds at minimum).
...
Reference:
- Apple docs: `CAMetalLayer` — `https://developer.apple.com/documentation/quartzcore/cametallayer`
...
Reference:
- Apple docs: `metal` tool — `https://developer.apple.com/documentation/metal/metal_shading_language/compiling_shaders_and_metal_libraries`
...
- `apple/swift-metal-tools` plugin docs (MetalCompilerPlugin) — `https://github.com/apple/swift-metal-tools/tree/main/Plugins/MetalCompilerPlugin`
...
- Might help with: framebuffer upload path (scaling, color conversion, blitting).
...
There isn’t a single “default” 3D physics engine for Swift + Metal. Options depend on goals:
...
- SceneKit includes a physics engine (historically Bullet-backed) with `SCNPhysicsWorld`, bodies, shapes.
- Pros: native framework, stable, quick to integrate.
- Cons: designed for SceneKit rendering; using it as “physics-only” requires manual transform syncing.

Reference:
- Apple docs: `SCNPhysicsWorld` — `https://developer.apple.com/documentation/scenekit/scnphysicsworld`
...
Reference:
- Apple docs: SpriteKit Physics — `https://developer.apple.com/documentation/spritekit/physics_simulation`
...
- Integration note: ships as an XCFramework; evaluate whether it fits the “no Xcode project” workflow (SwiftPM can still consume binary targets).
...
- Note: `box2d-swift` currently lacks an explicit license in the repo (treat as “investigate before use”).
...
- Jolt is attractive for performance and modern design, but Swift bindings are not commonly “off the shelf”.
...
- Physics: **none** or minimal custom primitives (AABB/sphere tests) until the render bridge is stable.

### Phase 2 (choose physics path)

- If 3D rigid body matters soon: evaluate **Bullet via PhysicsKit**.
- If we just need fast “game-ish” movement/collisions: implement custom collision queries first, then swap later.
- Keep the physics API behind a small adapter so we can switch engines without rewriting WASM contracts.
...
- [ ] Decide shader compilation workflow for the native host (CLI vs `MetalCompilerPlugin`).
