...
- MDN: `WebAssembly.Memory` reference (linear memory, grow behavior, endian
  notes)\
...
- MDN: `DedicatedWorkerGlobalScope.postMessage()` (worker → main thread
  messaging)\
  https://developer.mozilla.org/en-US/docs/Web/API/DedicatedWorkerGlobalScope/postMessage

## Synchronous XHR Deprecation (Context)

- WHATWG XHR spec note on sync XHR warning/deprecation pressure (see
  `sync-warning`)\
  https://xhr.spec.whatwg.org/#sync-warning
- Chrome Platform Status: “Deprecate and remove: synchronous XHR in page
  dismissal”\
...
- Swift Forums: “SwiftWasm deprecated?” (community context; why Swift now has
  native WASI support)\
