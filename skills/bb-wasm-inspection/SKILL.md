...
1. **Basic validity**
   ...
   - Fallback (parses module only):
     `deno run --allow-read Tools/wasm_validate.ts /tmp/program.wasm`

2. **Imports/exports are reasonable**
   - List what the web runtime/interpreter already implements vs what the module
     requires:
...
- Export surface matches intent:
  ...
  - Track B cmdbuf builds may also export
    `__CmdBufPtr/__CmdBufBytes/__CmdBufAbiVersion` (see
    `track-b-wasm-sanity-check` skill).
- Import surface doesn’t include unexpected game/user functions (import leaks
  are correctness bugs).
- Debug builds:
  - `.map` exists when compiled with `-g`
  - `.bbdbg.json` exists and/or the module imports
    `bbdbg.__bbdbg_enter/leave/stmt` when compiled with `-d`
