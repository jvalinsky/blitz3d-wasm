...
class Parser {
  ...
  statement() {
    ...
    if (
      this.check(
        lexer_1.TokenType.NEXT,
        lexer_1.TokenType.WEND,
        lexer_1.TokenType.UNTIL,
        lexer_1.TokenType.FOREVER,
        lexer_1.TokenType.CASE,
        lexer_1.TokenType.DEFAULT,
        lexer_1.TokenType.ENDIF,
        lexer_1.TokenType.END,
      )
    ) {
      return null;
    }
  ...
  functionDeclaration() {
    const name =
      this.consume(lexer_1.TokenType.IDENTIFIER, "Expected function name")
        .value;
    ...
    if (!this.check(lexer_1.TokenType.RPAREN)) {
      do {
        const paramName =
          this.consume(lexer_1.TokenType.IDENTIFIER, "Expected parameter name")
            .value;
  ...
  typeDeclaration() {
    const name =
      this.consume(lexer_1.TokenType.IDENTIFIER, "Expected type name").value;
    this.consumeNewlines();
    const fields = [];
    while (!this.check(lexer_1.TokenType.END) && !this.isAtEnd()) {
      ...
      if (this.check(lexer_1.TokenType.FIELD)) {
        this.advance();
        const fieldName =
          this.consume(lexer_1.TokenType.IDENTIFIER, "Expected field name")
            .value;
  ...
  variableDeclaration() {
    const scope = this.previous().type === lexer_1.TokenType.LOCAL
      ? "local"
      : this.previous().type === lexer_1.TokenType.GLOBAL
      ? "global"
      : this.previous().type === lexer_1.TokenType.CONST
      ? "const"
      : "dim";
    const name =
      this.consume(lexer_1.TokenType.IDENTIFIER, "Expected variable name")
        .value;
  ...
  ifStatement() {
    ...
    while (
      !this.check(
        lexer_1.TokenType.ELSE,
        lexer_1.TokenType.ELSEIF,
        lexer_1.TokenType.ENDIF,
        lexer_1.TokenType.END,
      ) && !this.isAtEnd()
    ) {
    ...
    if (this.match(lexer_1.TokenType.ELSE)) {
      ...
      elseBranch = [];
      while (
        !this.check(lexer_1.TokenType.ENDIF, lexer_1.TokenType.END) &&
        !this.isAtEnd()
      ) {
        const stmt = this.statement();
        if (stmt) {
  ...
  forLoop() {
    const variable =
      this.consume(lexer_1.TokenType.IDENTIFIER, "Expected loop variable")
        .value;
  ...
  whileLoop() {
    ...
    this.consume(
      lexer_1.TokenType.WEND,
      'Expected "Wend" after while loop body',
    );
  ...
  repeatLoop() {
    ...
    const body = [];
    while (
      !this.check(lexer_1.TokenType.UNTIL, lexer_1.TokenType.FOREVER) &&
      !this.isAtEnd()
    ) {
      const stmt = this.statement();
      if (stmt) {
  ...
  selectStatement() {
    ...
    while (this.match(lexer_1.TokenType.CASE) && !this.isAtEnd()) {
      if (this.check(lexer_1.TokenType.DEFAULT)) {
        ...
        defaultCase = [];
        while (
          !this.check(lexer_1.TokenType.CASE, lexer_1.TokenType.END) &&
          !this.isAtEnd()
        ) {
          const stmt = this.statement();
          if (stmt) {
    ...
    this.consume(
      lexer_1.TokenType.END,
      'Expected "End" after select statement',
    );
  ...
  comparison() {
    ...
    while (
      this.match(
        ...
        lexer_1.TokenType.LE,
        lexer_1.TokenType.GT,
        lexer_1.TokenType.GE,
      )
    ) {
      const operator = this.previous().value;
  ...
  multiplicative() {
    ...
    while (
      this.match(
        lexer_1.TokenType.MULTIPLY,
        lexer_1.TokenType.DIVIDE,
        lexer_1.TokenType.MOD,
      )
    ) {
      const operator = this.previous().value;
