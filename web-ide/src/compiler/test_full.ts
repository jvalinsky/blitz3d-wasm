...
function test(name: string, code: string, check: (ast: any, wat: string) => void) {
  try {
...
test('Integer variable declaration', `x% = 42`, (ast, wat) => {
  ...
  if (ast.statements[0].type !== 'Assignment') throw new Error('Expected Assignment');
  if (!wat.includes('i32.const 42')) throw new Error('Missing i32.const 42');
});

test('Float variable declaration', `x# = 3.14`, (ast, wat) => {
  if (ast.statements[0].value.type !== 'FloatLiteral') throw new Error('Expected FloatLiteral');
  if (!wat.includes('f32.const')) throw new Error('Missing f32.const');
});

test('String variable declaration', `x$ = "hello"`, (ast, wat) => {
  if (ast.statements[0].value.type !== 'StringLiteral') throw new Error('Expected StringLiteral');
});
...
test('Mixed operations', `Print 1 + 2 * 3 - 4`, (ast, wat) => {
  ...
  if (ast.statements[0].expression.type !== 'FunctionCall') throw new Error('Expected FunctionCall');
  if (ast.statements[0].expression.arguments[0].type !== 'BinaryOp') throw new Error('Expected BinaryOp in Print argument');
});
...
test('Simple If', `If x = 1 Then Print 1 EndIf`, (ast, wat) => {
  if (ast.statements[0].type !== 'IfStatement') throw new Error('Expected IfStatement');
...
test('If/ElseIf/Else', `If x = 1 Then Print "A" ElseIf x = 2 Then Print "B" Else Print "C" EndIf`, (ast, wat) => {
  if (ast.statements[0].elseIfBranches.length !== 1) throw new Error('Expected 1 ElseIf');
});
...
test('While loop', `While x < 10\n  x = x + 1\nWend`, (ast, wat) => {
  if (ast.statements[0].type !== 'WhileLoop') throw new Error('Expected WhileLoop');
...
test('Repeat Until', `Repeat\n  x = x + 1\nUntil x = 10`, (ast, wat) => {
  if (ast.statements[0].type !== 'RepeatStatement') throw new Error('Expected RepeatStatement');
...
test('Function declaration', `Function Add%(a%, b%)\n  Return a + b\nEnd Function`, (ast, wat) => {
  if (ast.statements[0].type !== 'FunctionDeclaration') throw new Error('Expected FunctionDeclaration');
  if (ast.statements[0].name !== 'Add%') throw new Error('Expected function name Add%');
});

test('Function with return', `Function Double%(x%)\n  Return x * 2\nEnd Function`, (ast, wat) => {
  if (!wat.includes('local.get $x')) throw new Error('Missing local.get for parameter');
});
...
test('For loop basic', `For i% = 1 To 5\n  Print i\nNext`, (ast, wat) => {
  ...
  if (ast.statements[0].variable !== 'i%') throw new Error('Expected variable i%');
});

test('For loop with step', `For i% = 1 To 10 Step 2\n  Print i\nNext`, (ast, wat) => {
...
test('Print string', `Print "Hello"`, (ast, wat) => {
  if (!wat.includes('call $printString')) throw new Error('Missing call $printString');
});
...
test('Data statement', `Data 1, 2, 3`, (ast, wat) => {
  if (ast.statements[0].type !== 'DataStatement') throw new Error('Expected DataStatement');
});
...
test('Complete program compiles', `
x% = 5
y% = 10
z% = x + y
Print z
...
test('WAT contains data section for strings', `Print "hello"`, (ast, wat) => {
  if (!wat.includes('(data')) throw new Error('Missing data section for strings');
});
