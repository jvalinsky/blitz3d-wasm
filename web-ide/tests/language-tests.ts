  ...
  for (const test of tests) {
    try {
      ...
      if (tokenResult.errors.length > 0 && test.shouldCompile) {
        throw new Error(`Lexer errors: ${tokenResult.errors.map(e => e.message).join(', ')}`);
      }
      
      if (test.expectedTokens && tokenResult.tokens.length !== test.expectedTokens) {
        console.log(`  ⚠️  Expected ${test.expectedTokens} tokens, got ${tokenResult.tokens.length}`);
      }
      ...
      if (test.expectedStatements && ast.statements.length !== test.expectedStatements) {
        console.log(`  ⚠️  Expected ${test.expectedStatements} statements, got ${ast.statements.length}`);
      }
