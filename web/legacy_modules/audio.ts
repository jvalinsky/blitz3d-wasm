...
class Blitz3DAudio {
  ...
  setupImports(imports) {
    ...
    imports.env.FSOUND_Init = (mixrate, maxchannels, flags) => {
      console.log(
        `FSOUND_Init: mixrate=${mixrate}, maxchannels=${maxchannels}, flags=${flags}`,
      );
    ...
    imports.env.FSOUND_Stream_Open = (pathPtr, mode, offset, length) => {
      ...
      console.log(
        `FSOUND_Stream_Open: ${path}, mode=${mode}, offset=${offset}, length=${length}`,
      );
      ...
      fetch(path)
        ...
        .then((audioBuffer) => {
          ...
          console.log(
            `Stream ${streamId} loaded: ${path} (${audioBuffer.duration}s)`,
          );
        })
        .catch((error) => {
    ...
    imports.env.FSOUND_Stream_GetTime = (streamId) => {
      ...
      for (const [channelId, channel] of this.channels.entries()) {
        if (channel.stream === streamId && channel.playing) {
          return Math.floor(
            (this.audioContext.currentTime - channel.startTime) * 1000,
          );
        }
      }
      return 0;
    ...
    imports.env.FSOUND_Stream_SetTime = (streamId, ms) => {
      console.warn(
        "FSOUND_Stream_SetTime not fully implemented (seeking requires source recreation)",
      );
    };
