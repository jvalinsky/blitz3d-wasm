...
class Blitz3DTextureLoader {
        ...
        for (let y = 0; y < texHeight; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                if (bitsPerPixel === 24) {
                    ...
                    const color16 = data[pixelOffsetBytes] | (data[pixelOffsetBytes + 1] << 8);
                    r = ((color16 >> 10) & 31) * 8;
                    g = ((color16 >> 5) & 31) * 8;
                    b = (color16 & 31) * 8;
    ...
    loadDDSTexture(data, options = {}) {
        ...
        if (compression) {
            console.warn(`DDS compression ${compression} not fully supported, using placeholder`);
            return this.placeholderTexture.clone();
        }
    ...
    applyTextureOptions(texture, options = {}) {
        ...
        // Anisotropy
        texture.anisotropy = Math.min(this.maxAnisotropy, 
