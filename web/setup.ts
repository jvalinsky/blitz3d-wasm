...
for (const filename of rootFiles) {
    ...
    try {
        ...
        console.warn(`Missing required root file: ${filename} (looked for ${dest} and ${src})`);
    }
}
...
if (convertAssets) {
    console.log(`[setup] converting source model assets to .smpk (${deleteSource ? "and deleting sources" : "keeping sources"})...`);
    const repoRoot = new URL("../", import.meta.url).pathname;
    const args = ["deno", "run", "-A", "Tools/assets_scpcb_convert.ts", "--root", "web/public", "--no-rewrite-manifest", "--no-validate-manifest"];
...
const manifest = {
    ...
    groups: {
        ...
        init: files
            ...
            .map((f) => f.path),
        facility_assets: files.filter((f) => f.path.startsWith("assets/") || f.path.startsWith("GFX/") || f.path.startsWith("SFX/") || f.path.startsWith("Data/")).map((f) => f.path)
    },
    files
};
