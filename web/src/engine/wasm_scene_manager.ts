...
export class WasmSceneManager {
    ...
    linePick(x: number, y: number, z: number, dx: number, dy: number, dz: number): { id: number, t: number, nx: number, ny: number, nz: number } | null {
        let closestId = 0;
    ...
    cameraPick(camId: number, x: number, y: number): { id: number, x: number, y: number, z: number, nx: number, ny: number, nz: number } | null {
        ...
        // Call LinePick
        const hit = this.linePick(origin.x, origin.y, origin.z, dest.x, dest.y, dest.z);
        if (hit) {
            // Calculate hit point
            const hitPoint = origin.clone().add(dir.clone().multiplyScalar(hit.t * range)); // Wait, t is 0..1 along line?
            ...
            const hitPointActual = origin.clone().add(dir.clone().normalize().multiplyScalar(hit.t));
