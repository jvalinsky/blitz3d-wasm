...
class FakeAudioContext {
  ...
  decodeImpl: () => Promise<AudioBuffer> = () =>
...
Deno.test("Audio handles stay stable across async decode + pause/resume", async () => {
  ...
  const audio = new Blitz3DAudio(core, {
  ...
  assertEquals(
    audio.isChannelPlaying(channelId),
    true,
    "pending channel should count as playing",
  );
  ...
  assertEquals(
    audio.isChannelPlaying(channelId),
    true,
    "channel should still be playing after decode",
  );
  ...
  assertEquals(
    audio.isChannelPlaying(channelId),
    false,
    "paused channel should not be playing",
  );
  ...
  assertEquals(
    audio.isChannelPlaying(channelId),
    true,
    "resume keeps channel id stable",
  );
});
