...
export class Blitz3DAudio {
    ...
    constructor(
        private core: any,
        opts?: { context?: AudioContext; createContext?: () => AudioContext | null },
    ) {
    ...
    private readSoundBytes(path: string): Uint8Array | null {
        ...
        const tryOpen = (p: string) => {
            ...
            try {
                ...
                try { fileIO.closeFile(handle); } catch { }
                return null;
            }
        };
    ...
    playStream(streamId: number, volume?: number, pan?: number, rate?: number, loopOverride?: boolean): number {
        return this._playFromEntry("stream", streamId, volume, pan, rate, loopOverride, false);
    }
    ...
    private startChannel(entry: ChannelEntry, buffer: AudioBuffer) {
        ...
        if (entry.is3D && entry.panner instanceof PannerNode) {
            ...
            if (pos) {
                entry.panner.positionX.setTargetAtTime(pos.x, this.context.currentTime, 0.02);
                entry.panner.positionY.setTargetAtTime(pos.y, this.context.currentTime, 0.02);
                entry.panner.positionZ.setTargetAtTime(pos.z, this.context.currentTime, 0.02);
            }
        }
    ...
    playSound(soundId: number, volume?: number, pan?: number, rate?: number, loopOverride?: boolean): number {
        return this._playFromEntry("sound", soundId, volume, pan, rate, loopOverride, false);
    }
    ...
    private _playFromEntry(
        ...
        const entry = kind === "sound" ? this.sounds.get(sourceId) : this.streams.get(sourceId);
        if (!entry) return 0;
