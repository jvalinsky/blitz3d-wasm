...
export class B3DLoader {
    ...
    parseBinaryData(data: Uint8Array) {
        this.data = data;
        this.offset = 0;
        this.textures = [];
        this.brushes = [];
    ...
    readBoneKeys(chunkSize: number) {
        ...
        while (this.offset < (this.data ? this.data.length : 0) && this.offset < this.offset + chunkSize) {
    ...
    async createThreeJSObjects(b3dData: any, parentId: number) {
        const root = new THREE.Group();
        const rootId = this.graphics.nextEntityId++;
        this.graphics.entities[rootId] = root;
    ...
    async createMesh(meshData: any, b3dData: any, meshIndex: number) {
    ...
    async createMaterial(brush: any, b3dData: any) {
        let texture = null;
    ...
    createPlaceholder(parentId: number) {
