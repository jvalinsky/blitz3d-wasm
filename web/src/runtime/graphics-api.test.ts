...
import { createGraphicsAPI, isWebGPUSupported, isWebGL2Supported, getGraphicsInfo } from './graphics-factory.ts';
import { createStandardVertexLayout, toArrayBuffer, CompareFunction, BlendMode, CullMode, BufferUsage, TextureFormat } from './graphics-api.ts';
...
Deno.test("Graphics API: Feature detection", async () => {
    ...
    assertEquals(hasSupport, true, "At least WebGPU or WebGL2 should be supported");
...
Deno.test("Graphics API: Buffer operations", async () => {
    try {
        ...
        const buffer = api.createBuffer(toArrayBuffer(vertices), BufferUsage.Vertex);
        assertExists(buffer);
...
Deno.test("Graphics API: Texture operations", async () => {
    try {
        ...
        const texture = api.createTexture(width, height, TextureFormat.RGBA8, data.buffer);
        ...
        console.log(`⚠️  Skipping texture test: ${(e as Error).message || String(e)}`);
    }
});
...
Deno.test("Graphics API: Shader compilation", async () => {
    try {
        ...
        console.log(`⚠️  Skipping shader test: ${(e as Error).message || String(e)}`);
    }
});
...
Deno.test("Graphics API: Capabilities query", async () => {
    try {
        ...
        console.log(`⚠️  Skipping capabilities test: ${(e as Error).message || String(e)}`);
    }
});
...
Deno.test("Graphics API: State management", async () => {
    try {
        ...
        console.log(`⚠️  Skipping state test: ${(e as Error).message || String(e)}`);
    }
});
...
Deno.test("Graphics API: Frame management", async () => {
    try {
        ...
        console.log(`⚠️  Skipping frame test: ${(e as Error).message || String(e)}`);
    }
});
