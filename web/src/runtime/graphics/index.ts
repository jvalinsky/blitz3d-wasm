...
import { type CommandExecutor, dispatchCmd } from "../command_executor.ts";
import { EngineBridge, EngineExports } from "../../engine/bridge.ts";
...
export class Blitz3DGraphics implements Blitz3DGraphicsInterface {
  ...
  constructor(core: GraphicsCore) {
    ...
    this.animationSystem = new this.Blitz3DAnimation(
      this as any,
      core,
  ...
  setEngine(exports: EngineExports, shaderSources?: any): void {
    ...
    if (this.core.canvas) {
      ...
      if (shaderSources) {
        ...
        this.nativeManager = new SceneManager(
          bridge,
          this.core.canvas,
          shaderSources,
        );
  ...
  engineCreate(gameId: number, type: number, parentGameId?: number): number {
    ...
    if (!manager) return 0;
    const parentEngineId = parentGameId
      ? (this._engineIds.get(parentGameId) ?? 0)
      : 0;
  ...
  private cmdCreateEntityWithId(
    ...
    const aspect = canvas && canvas.height
  ...
  private cmdTurnEntity(
    ...
    try {
      ...
      const eid = this._engineIds.get(id) ?? 0;
      if (manager && eid) {
        manager.bridge.exports.EngineTurnEntity(eid, pitch, yaw, roll);
      }
    } catch {}
  }
  ...
  private cmdPlaySound(
    ...
    if (outChannelPtr && this.core?.memory) {
      try {
        new DataView(this.core.memory.buffer).setInt32(
          outChannelPtr,
          chan,
          true,
        );
  ...
  drainCommandBuffer() {
    ...
    const exec: CommandExecutor = {
      ...
      onSetTransform: (id, pos, rot, scl) =>
        this.cmdSetTransform(id, pos, rot, scl),
      ...
      onSetParent: (id, parent, global) =>
        this.cmdSetParent(id, parent, global),
      ...
      onEntityTexture: (entityId, textureId, frame, index) =>
        this.cmdEntityTexture(entityId, textureId, frame, index),
      onEntityColor: (entityId, r, g, b) =>
        this.cmdEntityColor(entityId, r, g, b),
