...
export function setup2D(graphics: Blitz3DGraphicsInterface, imports: any) {
    const getContext = (): CanvasRenderingContext2D | null => {
        ...
        if (img && img.type === "image") {
            ...
            if (!blitzImg.canvas) {
                ...
                if (img.element && img.loaded && img.element instanceof HTMLImageElement) {
    ...
    imports.env.js_Cls = () => {
        // Clear 3D
        if (graphics.renderer && ((graphics as any).currentBuffer === -1 || !(graphics as any).currentBuffer)) {
        ...
        if (ctx) {
            ...
            if ((graphics as any).currentBuffer > 0) {
                ctx.fillStyle = `rgba(${graphics.clearColor[0] * 255}, ${graphics.clearColor[1] * 255}, ${graphics.clearColor[2] * 255}, ${graphics.clearColor[3]})`;
    ...
    imports.env.js_Oval = (x: number, y: number, w: number, h: number, solid: number) => {
        const ctx = getContext();
        if (ctx) {
            const c = graphics.currentColor;
            ctx.fillStyle = `rgb(${c[0]},${c[1]},${c[2]})`;
    ...
    imports.env.js_Text = (x: number, y: number, txtPtr: number, cx: number, cy: number) => {
    ...
    const measureText = (txt: string) => {
        ...
        ctx.font = `${graphics.currentFontSize}px ${graphics.currentFont}`;
        const metrics = ctx.measureText(txt);
        const height = (metrics.actualBoundingBoxAscent || graphics.currentFontSize * 0.8) +
    ...
    imports.env.js_LoadFont = (namePtr: number, size: number, bold: number, italic: number, underline: number) => {
    ...
    imports.env.AAStringWidth = (txtPtr: number) => imports.env.js_StringWidth(txtPtr);
    imports.env.AAStringHeight = (txtPtr: number) => imports.env.js_StringHeight(txtPtr);
    imports.env.AASpritePosition = (id: number, x: number, y: number) => {
        const s = graphics.entities[id]; if (s) s.position.set(x, y, s.position.z);
    };
    imports.env.AASpriteScale = (id: number, x: number, y: number) => {
        const s = graphics.entities[id]; if (s) s.scale.set(x, y, s.scale.z || 1);
    };
}
