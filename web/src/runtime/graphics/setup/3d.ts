...
/**
 * Install 3D-related WASM imports.
 *
 * These imports implement the Blitz3D 3D engine surface on top of Three.js (or
 * the thin client engine path when `graphics.wasmManager` is present).
 *
 * Notes:
 * - Source model formats (`.b3d`, `.x`, `.rmesh`) are only allowed when
 *   `globalThis.__BLITZ3D_ALLOW_SOURCE_MODELS === true` (interpreter/exploration).
 * - The source-model loader path requires `graphics.core.fileIO` to be set.
 */
export function setup3D(graphics: Blitz3DGraphicsInterface, imports: any) {
  ...
  imports.env.CreateCamera = (parent: number) => {
    if (graphics.wasmManager) {
      // Thin Client Path
      const id = graphics.wasmManager.createEntity(
        ENGINE_ENTITY_TYPE.CAMERA,
        parent,
      );
  ...
  imports.blitz3d.ParseRMesh = (bankId: number) => {
    ...
    // Lazy init loader
    if (!graphics.smpkLoader) {
      graphics.smpkLoader = new SMPKLoader(graphics, graphics.core);
    }

    // We load it as a child of the world (0). It returns a new entity ID.
    const rootId = graphics.smpkLoader.loadFromBytes(
      new Uint8Array(data.buffer, data.byteOffset, data.byteLength),
      0,
      `rmesh_${bankId} `,
    );
  ...
  imports.env.PositionEntity = (
    ...
    graphics.engineCall(
      ent,
      (eid) => graphics._engine!.EngineSetPosition(eid, x, y, z),
    );
  };
  ...
  imports.env.RotateEntity = (
    ...
    graphics.engineCall(
      ent,
      (eid) => graphics._engine!.EngineSetRotation(eid, pitch, yaw, roll),
    );
  };
  ...
  imports.env.ScaleEntity = (ent: number, x: number, y: number, z: number) => {
    ...
    graphics.engineCall(
      ent,
      (eid) => graphics._engine!.EngineSetScale(eid, x, y, z),
    );
  };
  ...
  imports.env.MoveEntity = (ent: number, x: number, y: number, z: number) => {
    ...
    graphics.engineCall(
      ent,
      (eid) => graphics._engine!.EngineMoveEntity(eid, x, y, z),
    );
  };

  imports.env.TurnEntity = (
    ent: number,
    pitch: number,
    yaw: number,
    roll: number,
    ...
    if (entity) {
      ...
      if (graphics.core.entityTable) {
        graphics.core.entityTable.setPitch(
          ent,
          entity.rotation.x * 180 / Math.PI,
        );
        graphics.core.entityTable.setYaw(
          ent,
          entity.rotation.y * 180 / Math.PI,
        );
        graphics.core.entityTable.setRoll(
          ent,
    ...
    graphics.engineCall(
      ent,
      (eid) => graphics._engine!.EngineTurnEntity(eid, pitch, yaw, roll),
    );
  };
  ...
  const applyToRenderable = (entity: any, fn: (obj: any) => void) => {
    if (!entity) return;
  ...
  imports.env.EntityColor = (ent: number, r: number, g: number, b: number) => {
    const entity = graphics.entities[ent];
    if (!entity) return;
    const color = new THREE.Color((r | 0) / 255, (g | 0) / 255, (b | 0) / 255);
  ...
  imports.env.EntityAlpha = (ent: number, a: number) => {
    const entity = graphics.entities[ent];
    if (!entity) return;
  ...
  imports.env.EntityFX = (ent: number, fx: number) => {
    // NOTE: Blitz3D's EntityFX is a bitfield. For interpreter/demo parity we
    // only implement the subset we actively use:
    // - bit 1: wireframe (runtime-defined in our demos)
    const entity = graphics.entities[ent];
    if (!entity) return;
    const flags = fx | 0;
  ...
  imports.env.EntityParent = (
    childId: number,
    parentId: number,
    global: number = 0,
  ) => {
    const child = graphics.entities[childId];
    if (!child) return 0;
    ...
    if (keepGlobal && worldMatrix) {
      try {
        ...
        if ((parent as any).matrixWorld) {
          inv.copy((parent as any).matrixWorld).invert();
        } else {
          inv.identity();
        }
  ...
  imports.env.EntityX = (ent: number, global: number) => {
    if (!global && graphics.core.entityTable) {
      return graphics.core.entityTable.getX(ent);
    }
    const entity = graphics.entities[ent];
  ...
  imports.env.EntityY = (ent: number, global: number) => {
    if (!global && graphics.core.entityTable) {
      return graphics.core.entityTable.getY(ent);
    }
    const entity = graphics.entities[ent];
    if (!entity) return 0.0;
    if (global) {
  ...
  imports.env.EntityZ = (ent: number, global: number) => {
    if (!global && graphics.core.entityTable) {
      return graphics.core.entityTable.getZ(ent);
    }
    const entity = graphics.entities[ent];
    if (!entity) return 0.0;
  ...
  imports.env.EntityPitch = (ent: number, global: number) => {
    if (!global && graphics.core.entityTable) {
      return graphics.core.entityTable.getPitch(ent);
    }
    const entity = graphics.entities[ent];
    if (!entity) return 0.0;
    if (global) {
  ...
  imports.env.EntityYaw = (ent: number, global: number) => {
    if (!global && graphics.core.entityTable) {
      return graphics.core.entityTable.getYaw(ent);
    }
    const entity = graphics.entities[ent];
    if (!entity) return 0.0;
    if (global) {
  ...
  imports.env.EntityRoll = (ent: number, global: number) => {
    if (!global && graphics.core.entityTable) {
      return graphics.core.entityTable.getRoll(ent);
    }
    const entity = graphics.entities[ent];
    if (!entity) return 0.0;
    if (global) {
  ...
  imports.env.CreatePivot = (parent: number) => {
    if (graphics.wasmManager) {
      return graphics.wasmManager.createEntity(
        ENGINE_ENTITY_TYPE.PIVOT,
        parent,
      );
    }
  ...
  imports.env.FreeEntity = (ent: number) => {
    ...
    if (entity) {
      ...
      graphics.engineCall(
        ent,
        (eid) => graphics._engine!.EngineFreeEntity(eid),
      );
  ...
  imports.env.EntityTexture = (
    ...
    graphics.engineCall(
      ent,
      (eid) =>
        graphics._engine!.EngineEntityTexture(eid, tex, frame || 0, index || 0),
    );
  };
  ...
  imports.env.LoadMesh = (pathPtr: number, parent: number) => {
    ...
    const allowSourceModels = globalThis.__BLITZ3D_ALLOW_SOURCE_MODELS === true;
    const rawLower = rawPath.toLowerCase();
    const isSource = rawLower.endsWith(".b3d") || rawLower.endsWith(".x") ||
      rawLower.endsWith(".rmesh");
    const path = (allowSourceModels && isSource)
      ? rawPath
      : rawPath.replace(/\.(b3d|x|rmesh)$/i, ".smpk");
    ...
    if (lowerPath.endsWith(".smpk")) {
      ...
      if (!allowSourceModels) {
        throw new Error(
          `Refusing to load source mesh at runtime: ${rawPath} (convert offline to .smpk)`,
        );
      }

      // Interpreter/exploration path: load source formats directly from VFS.
      (async () => {
        try {
          ...
          if (lowerPath.endsWith(".x")) {
            const { XLoader } = await import("../../xloader.ts");
            const x = new XLoader(
              graphics,
              coreWithFileIO,
              fileIO,
