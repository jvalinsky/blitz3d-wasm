...
const ensureAnimState = (entity: any) => {
  ...
  if (typeof entity.userData.__b3d_animFrame !== "number") {
    entity.userData.__b3d_animFrame = 0;
  }
  if (typeof entity.userData.__b3d_animPlaying !== "number") {
    entity.userData.__b3d_animPlaying = 0;
  }
  if (typeof entity.userData.__b3d_animSeq !== "number") {
    entity.userData.__b3d_animSeq = 0;
  }
};
...
export function setupAnimation(
  ...
  if (!imports.env.LoadAnimMesh) {
    imports.env.LoadAnimMesh = (pathPtr: number, parent: number) => {
      if (typeof imports.env.LoadMesh === "function") {
  ...
  if (!imports.env.LoadAnimMesh_Strict) {
    imports.env.LoadAnimMesh_Strict = (pathPtr: number, parent: number) => {
      if (typeof imports.env.LoadMesh_Strict === "function") {
        return imports.env.LoadMesh_Strict(pathPtr, parent);
      }
      if (typeof imports.env.LoadAnimMesh === "function") {
  ...
  if (!imports.env.Animate) {
    imports.env.Animate = (
      ...
      if (anim && typeof anim.animate === "function") {
        anim.animate(ent | 0, mode, speed, seq, trans);
      }
    };
  }
  ...
  if (!imports.env.SetAnimTime) {
    ...
    imports.env.SetAnimTime = (ent: number, time: number, seq: number) => {
      ...
      const anim: any = graphics.animationSystem;
      if (anim && typeof anim.setAnimTime === "function") {
        anim.setAnimTime(ent | 0, time, seq | 0);
      }
    };
  }
  ...
  if (!imports.env.AnimTime) {
    imports.env.AnimTime = (ent: number) => {
      const anim: any = graphics.animationSystem;
      if (anim && typeof anim.getAnimTime === "function") {
        return anim.getAnimTime(ent | 0);
      }
  ...
  if (!imports.env.AnimLength) {
    imports.env.AnimLength = (ent: number) => {
      const anim: any = graphics.animationSystem;
      if (anim && typeof anim.getAnimLength === "function") {
        return anim.getAnimLength(ent | 0);
      }
      return 0;
    };
  }
  ...
  if (!imports.env.ExtractAnimSeq) {
    imports.env.ExtractAnimSeq = (
      _ent: number,
      _first: number,
      _last: number,
    ) => 0;
  }
  if (!imports.env.AddAnimSeq) {
  ...
  for (const k of mirror) {
    if (!imports.blitz3d[k] && imports.env[k]) {
      imports.blitz3d[k] = imports.env[k];
    }
  }
}
