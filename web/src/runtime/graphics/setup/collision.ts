...
export function setupCollision(
  ...
  imports.env.Collisions = (
    ...
    if (graphics.wasmManager) {
      graphics.wasmManager.bridge.collisions(
        srcType,
        destType,
        method,
        response,
  ...
  imports.env.CollisionX = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionX(entId, index)
      : 0;
  imports.env.CollisionY = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionY(entId, index)
      : 0;
  imports.env.CollisionZ = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionZ(entId, index)
      : 0;
  imports.env.CollisionNX = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionNX(entId, index)
      : 0;
  imports.env.CollisionNY = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionNY(entId, index)
      : 0;
  imports.env.CollisionNZ = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionNZ(entId, index)
      : 0;
  imports.env.CollisionEntity = (entId: number, index: number) =>
    graphics.wasmManager
      ? graphics.wasmManager.bridge.collisionEntity(entId, index)
      : 0;
