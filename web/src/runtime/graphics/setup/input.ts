...
export function setupInput(graphics: Blitz3DGraphicsInterface, imports: any) {
  ...
  imports.env.KeyDown = (key: number) => {
    return (graphics.inputManager && graphics.inputManager.keysDown[key])
      ? 1
      : 0;
  };
  ...
  imports.env.MouseDown = (btn: number) => {
    return (graphics.inputManager && graphics.inputManager.mouseDown[btn])
      ? 1
      : 0;
  };
  ...
  imports.env.MouseX = () =>
    graphics.inputManager ? graphics.inputManager.mouseX : 0;
  imports.env.MouseY = () =>
    graphics.inputManager ? graphics.inputManager.mouseY : 0;
  imports.env.MouseZ = () =>
    graphics.inputManager ? graphics.inputManager.mouseZ : 0;
  imports.env.MouseXSpeed = () =>
    graphics.inputManager ? graphics.inputManager.mouseXSpeed : 0;
  imports.env.MouseYSpeed = () =>
    graphics.inputManager ? graphics.inputManager.mouseYSpeed : 0;
  imports.env.MouseZSpeed = () =>
    graphics.inputManager ? graphics.inputManager.mouseZSpeed : 0;
