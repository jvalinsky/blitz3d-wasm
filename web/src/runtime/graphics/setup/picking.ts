...
export function setupPicking(
  ...
  const updatePickResult = (
    ...
    while (curr) {
      if (curr.userData && curr.id) {
        for (let id in graphics.entities) {
          if (
            graphics.entities[id] === curr ||
            graphics.entities[id] === curr.parent
          ) {
    ...
    if (graphics.lastPick) {
      ...
      const faceIndex =
        (intersect as unknown as { faceIndex?: number }).faceIndex;
      graphics.lastPick.triangle = Number.isFinite(faceIndex)
        ? (faceIndex as number)
        : 0;
      const surfId = intersect.object.userData?.surfaceId ??
        intersect.object.userData?.surface ??
        intersect.object.userData?.surfaceIndex;
      graphics.lastPick.surface =
        (typeof surfId === "number" && Number.isFinite(surfId)) ? surfId : 0;
    }
    return pickedEnt;
  };
  ...
  imports.env.LinePick = (
    ...
    if (graphics.wasmManager) {
      ...
      if (hit) {
        ...
        const hitPos = new THREE.Vector3(x, y, z).add(
          dir.multiplyScalar(hit.t),
        );
        graphics.lastPick = {
          entity: hit.id,
          x: hitPos.x,
  ...
  imports.env.PickedEntity = () =>
    graphics.lastPick ? graphics.lastPick.entity : 0;
  imports.env.PickedSurface = () =>
    graphics.lastPick ? graphics.lastPick.surface : 0;
  imports.env.PickedTriangle = () =>
    graphics.lastPick ? graphics.lastPick.triangle : 0;
