...
export function setupSCPCB(graphics: Blitz3DGraphicsInterface, imports: any) {
  ...
  imports.env.CreateDoor = (
    ...
    console.log(
      `[SCPCB Door] Created door at ${x},${y},${z} yaw=${yaw} locked=${locked}`,
    );
    return 0;
  };
  ...
  imports.env.CreateRoom = (
    ...
    console.log(
      `[SCPCB Room] Created room ${template} zone=${zone} at ${x},${y},${z} yaw=${yaw}`,
    );
    return 0;
  };
  ...
  imports.env.DrawButton = (
    ...
    if (imports.env.Text) {
      imports.env.Text(x + width / 2, y + height / 2, text, 1, 1);
    }
    return 1;
  };

  /** Creates a button */
  imports.env.CreateButton = (
    x: number,
    y: number,
    width: number,
    height: number,
  ...
  imports.env.DrawTiledImageRect = (
    img: number,
    sx: number,
    sy: number,
    sw: number,
  ...
  imports.env.LoadFont_Strict = (
    ...
    if (imports.env.LoadFont) {
      return imports.env.LoadFont(name, size, bold, italic, under);
    }
    return 0;
  };
