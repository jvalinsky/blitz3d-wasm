...
Deno.test("VFS - EOF detection", () => {
  ...
  assertEquals(
    fileIO.eof(handle),
    1,
    "Should be at EOF after reading all bytes",
  );
...
Deno.test("File Operations - seek and tell", () => {
  ...
  assertEquals(
    fileIO.tell(handle),
    1,
    "Position should be 1 after reading 1 byte",
  );
...
Deno.test("Error Handling - dispatchErrorEvent fires event", () => {
  ...
  if (typeof window !== "undefined") {
    ...
    assertEquals(
      (eventDetail as { filePath: string }).filePath,
      "test/path.txt",
    );
  }
});
...
Deno.test("Error Handling - fetchWithRetry retries on server error", async () => {
  ...
  try {
    const response = await fileIO.fetchWithRetry(
...
Deno.test("GetINIInt reads int from INI", () => {
  ...
  const iniContent = new TextEncoder().encode(
    "[graphics]\nwidth=1024\nheight=768\n",
  );
