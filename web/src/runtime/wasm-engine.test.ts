...
import { loadBlitz3DEngine, isWasmSupported } from './wasm-loader.ts';

// Path to WASM file (relative to project root when tests run)
const WASM_PATH = './web/dist/blitz3d-engine.wasm';
...
Deno.test('WASM memory exports are available', async () => {
    ...
    assertEquals(typeof engine.exports.malloc, 'function', 'malloc should be a function');
    assertEquals(typeof engine.exports.free, 'function', 'free should be a function');
    
    // Memory should be non-empty
    const memSize = engine.exports.memory.buffer.byteLength;
    assertEquals(memSize > 0, true, `Memory should be non-empty (got ${memSize} bytes)`);
});
...
Deno.test('Bank operations: PokeFloat and PeekFloat', async () => {
    ...
    assertEquals(Math.abs(value - 3.14159) < epsilon, true, 
...
Deno.test('Math operations: Sin, Cos, Sqrt', async () => {
    ...
    const epsilon = 0.0001;
    assertEquals(Math.abs(sin90 - 1.0) < epsilon, true, 
        `Sin(π/2) should be ~1.0 (got ${sin90})`);
    
    // Test Cos
    const cos0 = engine.exports.Cos(0);
    assertEquals(Math.abs(cos0 - 1.0) < epsilon, true,
    ...
    assertEquals(Math.abs(sqrt2 - 1.41421) < 0.001, true,
