 ...
 * Phase 2: Graphics imports now implemented
 */
function createEngineImports(audio?: Blitz3DAudio, readString?: (ptr: number) => string): Blitz3DEngineImports {
    return {
        env: {
            ...
            js_EmitSound: (sound: number, entityId: number): number => {
                ...
                return audio.playSound3D(sound, 0, 0, 0, undefined, undefined, undefined, entityId);
            },
                ...
                if (audio) {
                    ...
                    if (entry) {
                        ...
                        if (entry.panner instanceof PannerNode && audio.context) {
                            entry.panner.positionX.setTargetAtTime(x, audio.context.currentTime, 0.02);
                            entry.panner.positionY.setTargetAtTime(y, audio.context.currentTime, 0.02);
                            entry.panner.positionZ.setTargetAtTime(z, audio.context.currentTime, 0.02);
                        }
                    }
                }
            },
...
export async function loadBlitz3DEngine(
    ...
    const strings = new WasmStringHelper(
        exports.memory,
        (exports.wasm_malloc as any) || (() => { throw new Error('wasm_malloc not exported'); }),
        (exports.wasm_free as any) || (() => { throw new Error('wasm_free not exported'); })
    );
