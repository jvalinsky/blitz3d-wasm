...
export class XLoader {
    ...
    constructor(graphics: any, core: any, fileIO: any) {
     ...
     */
    async loadFile(filePath: string, parentId = 0) {
     ...
     */
    findBlocks(content: string, blockType: string) {
        const blocks = [];
     ...
     */
    extractBlock(content: string, start: number) {
        let depth = 1;
        let i = start;
    ...
    parseVertices(data: string, count: number) {
        const vertices = [];
    ...
    parseFaces(data: string, count: number) {
        const faces = [];
    ...
    extractTextureName(block: string) {
     ...
     */
    async createThreeJSObjects(xData: any, parentId: number, filePath: string) {
        // Create root group
    ...
    async createMesh(meshData: any, filePath: string) {
